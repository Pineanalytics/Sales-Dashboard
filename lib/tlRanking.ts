...
/** Pre-aggregated Target contribution per Team Leader — one row per Team
 *  Leader, already summed by the caller. In practice this is sourced from the
 *  Target x RepContribution cascade (lib/mtdTarget.ts) — the full month's
 *  figure, not pro-rated. Kept as a generic pair here so this function stays
 *  pure/testable regardless of where the caller sourced it from. */
