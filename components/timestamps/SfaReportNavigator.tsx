  ...
  return (
    <section className="rounded-2xl border border-border bg-surface p-3 shadow-[0_4px_14px_rgba(11,61,53,0.08)] sm:p-4">
      ...
      <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
        {SYSTEMS.map((item) => {
          const active = item.principal === current;
          ...
          return item.status === "Live" ? <Link key={item.principal} href={item.href} className={className}>{content}</Link> : <div key={item.principal} className={`${className} opacity-80`}>{content}</div>;
        })}
      </div>
    </section>
  );
