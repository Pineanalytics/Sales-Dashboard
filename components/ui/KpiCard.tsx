...
export function KpiCard({ label, value, sublabel, accent = "revenue", icon, size = "lg", delta, sparkline }: KpiCardProps) {
  ...
  return (
    <div
      className={`h-full rounded-xl border-t-4 ${kpiAccentBorderClass[accent]} bg-surface p-3.5 flex flex-col gap-1 min-w-0 shadow-sm ring-1 ring-border/60 transition-all duration-300 hover:shadow-md hover:-translate-y-0.5`}
    >
