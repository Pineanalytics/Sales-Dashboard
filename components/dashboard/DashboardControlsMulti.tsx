...
/** Executive controls with a multi-principal-aware Clear action. */
export function DashboardControlsMulti({ view, onViewChange }: { view: DashboardView; onViewChange: (view: DashboardView) => void }) {
        ...
        <div className="flex min-w-0 flex-1 flex-wrap items-center gap-3">
          <div className="inline-flex shrink-0 rounded-xl bg-background-elevated p-1" aria-label="Executive summary view">
            {([ { key: "mtd", label: "MTD Sales" }, { key: "ytd", label: "YTD Summary" } ] as const).map((option) => (
              <button key={option.key} type="button" onClick={() => onViewChange(option.key)} className={`rounded-lg px-3 py-1.5 text-xs font-bold transition-colors ${view === option.key ? "bg-gradient-to-r from-primary-blue to-secondary-blue text-white shadow-cyan-glow" : "text-muted-strong hover:text-primary-blue"}`}>
                {option.label}
              </button>
            ))}
          </div>
