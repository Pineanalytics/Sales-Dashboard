  ...
  return <div className="flex flex-col gap-6">
    <SectionCard title="Principal KPI workspace" action={<span className="text-xs text-muted">Mars is the first configured principal; each view uses the Mars fiscal calendar.</span>}>
      <div className="flex flex-wrap items-end gap-4">
        ...
        <label className="grid gap-1 text-xs font-medium text-muted">Fiscal period<select value={selectedPeriod} onChange={(event) => setPeriod(Number(event.target.value))} className="h-10 min-w-60 rounded-xl border border-border bg-background-elevated px-3 text-sm font-semibold text-foreground">{periods.map((item) => <option value={item.periodNo} key={item.periodKey}>{item.periodKey} | {new Date(item.startDate).toLocaleDateString("en-GB", { day: "numeric", month: "short" })} - {new Date(item.endDate).toLocaleDateString("en-GB", { day: "numeric", month: "short" })}</option>)}</select></label>
    ...
    {slide === "overview" ? <><KpiGrid>
      ...
      <KpiCard accent="coverage" label="PTD SSU achievement" value={<AnimatedValue value={summary.ptdAchievement ?? 0} format={(value) => value === 0 && summary.ptdAchievement === null ? "-" : formatPercent(value)} />} sublabel={`Target: ${formatNumber(summary.target.ptdSsuTarget)}`} />
      <KpiCard accent="coverage" label="Coverage (PTD)" value={<AnimatedValue value={summary.ptdCoverage ?? 0} format={(value) => value === 0 && summary.ptdCoverage === null ? "-" : formatPercent(value)} />} sublabel={`${formatNumber(summary.current.ptdOutlets)} visited`} />
      <KpiCard accent="growth" label="Productive outlets (PTD)" value={<AnimatedValue value={summary.current.ptdProductive} format={formatNumber} />} sublabel={`Strike: ${summary.ptdStrikeRate === null ? "-" : formatPercent(summary.ptdStrikeRate)}`} />
      <KpiCard accent="quarter" label="Conversion rate (PTD)" value={<AnimatedValue value={summary.ptdConversion ?? 0} format={(value) => value === 0 && summary.ptdConversion === null ? "-" : formatPercent(value)} />} />
      ...
      <KpiCard accent="quarter" label="Fiscal YTD achievement" value={<AnimatedValue value={summary.ytdAchievement ?? 0} format={(value) => value === 0 && summary.ytdAchievement === null ? "-" : formatPercent(value)} />} sublabel={`Target: ${formatNumber(summary.target.ytdSsuTarget)}`} />
