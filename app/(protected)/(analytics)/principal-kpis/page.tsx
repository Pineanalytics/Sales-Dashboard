...
interface MarsData { principal: string; available: boolean; fiscalYear?: string; priorYear?: string; selectedPeriod?: number; source?: "PINE" | "WORKBOOK"; priorSource?: "PINE" | "WORKBOOK"; asOf?: string | null; periods?: { periodKey: string; periodNo: number; startDate: string; endDate: string }[]; summary?: Summary; filterOptions?: { sellerTypes: string[]; employeeGroups: string[]; locations: string[]; teamLeaders: string[]; fsrs: string[] }; byPeriod?: { periodKey: string; periodNo: number; ssu: number; revenue: number; outlets: number }[]; bySeller?: { name: string; ptdSsu: number; ytdSsu: number; ptdRevenue: number; ytdRevenue: number; ptdVisits: number; ptdProductive: number }[]; byBrand?: { name: string; ssu: number; revenue: number }[]; rtmPerformance?: { name: string; ptdSsu: number; ptdRevenue: number; ptdVisits: number; ptdProductive: number }[]; rtmUniverse?: { name: string; customers: number }[]; locationScorecards?: { location: string; sellerType: string; stream: string; fullSsuTarget: number; ptdSsuTarget: number; ptdSsu: number; lyspSsu: number; universeTarget: number; visits: number; lyspVisits: number; productive: number; lyspProductive: number; lppc: number | null; dropSize: number | null }[]; repProductivityScorecards?: { employeeCode: string; employeeName: string; location: string; target: number; productive: number }[]; }
      ...
      <div className="grid gap-5">
        {commercialStreams.map((stream) => {
          ...
          return <div key={stream} className="overflow-hidden rounded-xl border border-border">
            ...
            <TableWrap><Thead><Th>Location / sub-region</Th><Th align="right">Full target</Th><Th align="right">PTD target</Th><Th align="right">SSU sold PTD</Th><Th align="right">vs target</Th><Th align="right">SSU sold LYSP</Th><Th align="right">vs LYSP</Th><Th align="right">Universe</Th><Th align="right">Visits PTD</Th><Th align="right">Visits LYSP</Th><Th align="right">Coverage</Th><Th align="right">Coverage growth</Th><Th align="right">Productive PTD</Th><Th align="right">Productive LYSP</Th><Th align="right">Strike</Th><Th align="right">Conversion</Th><Th align="right">Productive growth</Th><Th align="right">L.P.P.C</Th><Th align="right">Drop size</Th></Thead><tbody>{rows.map((row) => <tr key={`${stream}-${row.location}`}>
              <Td>{row.location}</Td><Td align="right">{formatNumber(row.fullSsuTarget)}</Td><Td align="right">{formatNumber(row.ptdSsuTarget)}</Td><Td align="right">{formatNumber(row.ptdSsu)}</Td><Td align="right">{row.ptdSsuTarget > 0 ? formatPercent(row.ptdSsu / row.ptdSsuTarget) : "—"}</Td><Td align="right">{formatNumber(row.lyspSsu)}</Td><Td align="right">{row.lyspSsu > 0 ? formatPercent((row.ptdSsu / row.lyspSsu) - 1) : "—"}</Td><Td align="right">{formatNumber(row.universeTarget)}</Td><Td align="right">{formatNumber(row.visits)}</Td><Td align="right">{formatNumber(row.lyspVisits)}</Td><Td align="right">{row.universeTarget > 0 ? formatPercent(row.visits / row.universeTarget) : "—"}</Td><Td align="right">{row.lyspVisits > 0 ? formatPercent((row.visits / row.lyspVisits) - 1) : "—"}</Td><Td align="right">{formatNumber(row.productive)}</Td><Td align="right">{formatNumber(row.lyspProductive)}</Td><Td align="right">{row.visits > 0 ? formatPercent(row.productive / row.visits) : "—"}</Td><Td align="right">{row.universeTarget > 0 ? formatPercent(row.productive / row.universeTarget) : "—"}</Td><Td align="right">{row.lyspProductive > 0 ? formatPercent((row.productive / row.lyspProductive) - 1) : "—"}</Td><Td align="right">{row.lppc === null ? "—" : row.lppc.toLocaleString("en-US", { maximumFractionDigits: 1 })}</Td><Td align="right">{row.dropSize === null ? "—" : row.dropSize.toLocaleString("en-US", { maximumFractionDigits: 1 })}</Td>
            </tr>)}</tbody></TableWrap>
          </div>;
        })}
      </div>
