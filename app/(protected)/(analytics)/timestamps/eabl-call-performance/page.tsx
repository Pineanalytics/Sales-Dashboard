...
export default function EablCallPerformancePage() {
  ...
  return <main className="mx-auto flex max-w-[1600px] flex-col gap-5 px-4 py-5 sm:px-6 lg:px-8">
    ...
    <div className="flex flex-wrap items-start justify-between gap-3"><div><Link href="/timestamps" className="inline-flex items-center gap-1 text-xs font-semibold text-primary-blue hover:underline"><ArrowLeft20Regular className="h-4 w-4" /> All timestamp systems</Link><h1 className="mt-2 text-2xl font-bold text-brand-navy">EABL DMS · Call Performance</h1><p className="mt-1 text-sm text-muted">Dedicated call-level activity for {summary.scope}; separate from the SalesEdge timestamp feed.</p></div><div className="flex flex-wrap gap-2"><span className="rounded-full bg-accent-amber-soft px-3 py-1 text-xs font-semibold text-accent-amber">Data through {metrics.latestCallDate ? date(metrics.latestCallDate) : "—"}</span><span className="rounded-full bg-accent-blue-soft px-3 py-1 text-xs font-semibold text-primary-blue">{summary.syncUpdatedAt ? `Synced ${new Date(summary.syncUpdatedAt).toLocaleString("en-KE")}` : "Awaiting first sync"}</span></div></div>
