...
export default async function EmployeeMasterPage({ searchParams }: { searchParams: Promise<{ success?: string; error?: string; edit?: string; add?: string }> }) {
  ...
  const [employees, suggestions] = await Promise.all([
  ...
  return <div className="min-h-screen bg-background">
    ...
    <div className="mx-auto flex max-w-[1500px] flex-col gap-5 p-4 md:p-8">
      ...
      {showForm ? <section className="rounded-2xl border border-secondary-blue/30 bg-surface p-5 shadow-[0_6px_18px_rgba(11,61,53,0.1)]"><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-secondary-blue">{editing ? "Edit roster record" : "New roster record"}</p><h2 className="mt-1 text-lg font-bold text-brand-navy">{formTitle}</h2>{addingSuggestion ? <p className="mt-1 text-xs text-muted">Suggested from {addingSuggestion.productiveCalls.toLocaleString()} productive timestamp call(s) and {addingSuggestion.sales.toLocaleString("en-KE", { style: "currency", currency: "KES", notation: "compact" })} sold over the past 90 days.</p> : null}</div><Link href="/admin/employee-master" className="text-xs font-semibold text-primary-blue hover:underline">Cancel</Link></div><form action={saveEmployeeMasterAction} className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3"><label className={labelClass}>Employee code<input name="employeeCode" required readOnly={Boolean(editing)} defaultValue={editing?.employeeCode ?? addingSuggestion?.employeeCode ?? ""} className={`${inputClass} ${editing ? "bg-background-elevated text-muted" : ""}`} /></label><label className={labelClass}>Pine / SalesEdge name<input name="pineName" required defaultValue={editing?.pineName ?? addingSuggestion?.salesRep ?? ""} className={inputClass} /></label><label className={labelClass}>SAP sales rep name<input name="sapName" defaultValue={editing?.sapName ?? addingSuggestion?.salesRep ?? ""} className={inputClass} /></label><label className={labelClass}>Absolute principal<input name="absolutePrincipal" required defaultValue={editing?.absolutePrincipal ?? addingSuggestion?.suggestedPrincipal ?? ""} className={inputClass} /><span className="mt-1 block text-[11px] text-muted">A timestamp suggestion is pre-filled for review; amend it if needed.</span></label><label className={labelClass}>Sales role<select name="salesRole" defaultValue={editing?.salesRole ?? "Primary Sales"} className={inputClass}><option>Primary Sales</option><option>Secondary Sales</option></select></label>{isAdmin ? <label className={labelClass}>Team Leader<select name="teamLeader" required defaultValue={editing?.teamLeader ?? ""} className={inputClass}><option value="">Select Team Leader</option>{teamLeaderOptions.map((name) => <option key={name}>{name}</option>)}</select></label> : <label className={labelClass}>Team Leader<input readOnly value={teamLeader!.name} className={`${inputClass} bg-background-elevated text-muted`} /><input type="hidden" name="teamLeader" value={teamLeader!.name} /></label>}<div className="flex items-end"><button type="submit" className="w-full rounded-full bg-primary-blue px-4 py-2.5 text-sm font-semibold text-white hover:bg-secondary-blue">{editing ? "Save changes" : "Add to roster"}</button></div></form></section> : null}
