...
| --- | --- | --- | --- | --- | --- |
| Sales Dashboard application | Serves the dashboard, protected integration endpoints, and sync-health status. | App/API source in this repository; protected VPS environment files. | Docker Compose on the VPS. Deployed only with `scripts/deploy.ps1` from clean `master`. | VPS Postgres and `/api/health` deployment identity. | App changes require a PR, validation, and post-deploy health verification. `pinefrostdb.com` is canonical; `www.pinefrostdb.com` is TLS-enabled and permanently redirects to the apex hostname. |
...
| UKL Nairobi puller | Delivers a Nairobi snapshot into the watched server folder on every scheduled run. | `scripts/ukl-sales-export-pull.ps1` and `scripts/install-ukl-sales-export-schedule.ps1`; puller installed as `C:\ukl-sales-export-pull.ps1`. | `PINEFROSTSERVER` task **UKL-SalesExport-Pull**, hourly at :05 outside the 14:00–18:00 boost window; **UKL-SalesExport-Pull-Nairobi-Boost** every ten minutes from 14:00 through 17:50. | `D:\UKL_INTEGRATION\UPLOADS`; downstream archive at `D:\UKL_INTEGRATION\UPLOADS\Archive`. | Each saved run uses `UKL_NAIROBI_<DD.MM.YYYY>_<UTC date>_<UTC time and GUID>.csv`, such as `UKL_NAIROBI_12.09.2026_20260915_T1305154953584Z_<GUID>.csv`. The ID is generated at write time and never depends on files remaining in UPLOADS or Archive. It is intentionally staggered ahead of Nyeri and the shared puller mutex serializes file writes. Uses `UKL_SALES_EXPORT_KEY` and optionally `PIPELINE_ALERT_KEY`. |
| UKL Nyeri puller | Delivers a Nyeri snapshot into the watched server folder on every scheduled run. | `scripts/ukl-sales-export-pull.ps1` and `scripts/install-ukl-sales-export-schedule.ps1`; puller installed as `C:\ukl-sales-export-pull.ps1`. | `PINEFROSTSERVER` task **UKL-SalesExport-Pull-Nyeri**, hourly at :25 outside the 14:00–18:00 boost window; **UKL-SalesExport-Pull-Nyeri-Boost** every ten minutes from 14:05 through 17:55. | `D:\UKL_INTEGRATION\UPLOADS`; downstream archive at `D:\UKL_INTEGRATION\UPLOADS\Archive`. | Each saved run uses `UKL_NYERI_<DD.MM.YYYY>_<UTC date>_<UTC time and GUID>.csv`, such as `UKL_NYERI_12.09.2026_20260915_T1305154953584Z_<GUID>.csv`. The ID is generated at write time and never depends on files remaining in UPLOADS or Archive. The offset and shared puller mutex prevent competing writes. Uses `UKL_SALES_EXPORT_KEY` and optionally `PIPELINE_ALERT_KEY`. |
