...
| UKL Nairobi puller | Reconciles Nairobi UKL files into the watched server folder. | `scripts/ukl-sales-export-pull.ps1`; installed as `C:\ukl-sales-export-pull.ps1`. | `PINEFROSTSERVER` task **UKL-SalesExport-Pull**, every five minutes in Africa/Nairobi. | `D:\UKL_INTEGRATION\UPLOADS`; state at `D:\UKL_INTEGRATION\STATE\ukl-sales-export-NAIROBI.json`; downstream archive at `D:\UKL_INTEGRATION\UPLOADS\Archive`. | Processes today first and all content changes in one run. Previous-day close reconciliation runs through 06:00; a content change detected after 06:00 is treated as a late transaction and automatically re-exported. Uses `UKL_SALES_EXPORT_KEY` and optionally `PIPELINE_ALERT_KEY`. |
| UKL Nyeri puller | Reconciles Nyeri UKL files into the watched server folder. | `scripts/ukl-sales-export-pull.ps1`; installed as `C:\ukl-sales-export-pull.ps1`. | `PINEFROSTSERVER` task **UKL-SalesExport-Pull-Nyeri**, every five minutes in Africa/Nairobi, offset approximately two minutes from Nairobi. | `D:\UKL_INTEGRATION\UPLOADS`; state at `D:\UKL_INTEGRATION\STATE\ukl-sales-export-NYERI.json`; downstream archive at `D:\UKL_INTEGRATION\UPLOADS\Archive`. | Same content-change and 06:00 policy as Nairobi. Uses `UKL_SALES_EXPORT_KEY` and optionally `PIPELINE_ALERT_KEY`. |
