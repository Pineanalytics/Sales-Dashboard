...
- `GET /api/integrations/ukl/sales-export` — headerless CSV export of one day's, one distributor's `SalesReturnLine` rows (25 columns, matching the original hand-run report's exact order/format), reconciled every five minutes by `scripts/ukl-sales-export-pull.ps1` on the separate server that hosts `D:\UKL_INTEGRATION\UPLOADS`. Smart mode checks an exact Nairobi-calendar yesterday/today manifest, processes today first, and completes every changed day in the same run. Revisions are content-based: yesterday's routine close-period refresh effectively ends at 06:00 Africa/Nairobi, while a genuine late transaction detected later still replaces yesterday automatically. A hash-matching file moved into `D:\UKL_INTEGRATION\UPLOADS\Archive` (or the configured `-ArchiveFolder`) counts as delivered; older dates require an explicit manual `-Date`. Requires a numeric `distributor` plus `x-ukl-export-key`/`UKL_SALES_EXPORT_KEY`; branch filtering prevents Nairobi/Nyeri files from containing combined data.
...
- `GET/POST /api/sales-returns/reconciliation` — Smart-mode machine endpoint. `GET` returns exact VPS daily signatures (row/invoice counts, quantities, sales, and discounts) for one distributor and a bounded date range. Each Centegy machine compares these with the same aggregates from its SQL report, repairs the oldest missing/changed day, verifies it, then `POST`s a successful heartbeat. This lets next-day reconciliations and historical gaps self-heal without repeatedly extracting unchanged dates. API-key-only; requests are capped at 62 days.
...
- `POST /api/pipeline-alerts` — failure/issue-only email hook for the standalone Centegy and UKL export scripts. Successful high-frequency runs are acknowledged without email; failures send via `lib/email.ts` so the SMTP password stays only on the VPS. API-key-only (`x-pipeline-alert-key`, `PIPELINE_ALERT_KEY`) — never session-authenticated.
