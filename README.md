...
- `GET /api/integrations/ukl/sales-export` — headerless CSV export of one day's, one distributor's `SalesReturnLine` rows (25 columns, matching the original hand-run report's exact order/format), refreshed every five minutes by `scripts/ukl-sales-export-pull.ps1` on the separate server that hosts `D:\UKL_INTEGRATION\UPLOADS`. Requires `date` and numeric `distributor` query parameters plus `x-ukl-export-key`/`UKL_SALES_EXPORT_KEY`; branch filtering prevents Nairobi/Nyeri files from containing combined data.
