...
- `GET /api/integrations/ukl/sales-export` — headerless CSV export of one day's, one distributor's `SalesReturnLine` rows (25 columns, matching the original hand-run report's exact order/format), delivered by `scripts/ukl-sales-export-pull.ps1` on the separate server that hosts `D:\UKL_INTEGRATION\UPLOADS`. The scheduled cadence writes a new numbered snapshot every run; its next sequence is determined across both UPLOADS and Archive, so archival cannot restart numbering. Manual Smart mode checks an exact Nairobi-calendar yesterday/today manifest and only writes changed days. Requires a numeric `distributor` plus `x-ukl-export-key`/`UKL_SALES_EXPORT_KEY`; branch filtering prevents Nairobi/Nyeri files from containing combined data.
