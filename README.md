...
- `GET /api/integrations/ukl/sales-export` — headerless CSV export of one day's, one distributor's `SalesReturnLine` rows (25 columns, matching the original hand-run report's exact order/format), refreshed every five minutes by `scripts/ukl-sales-export-pull.ps1` on the separate server that hosts `D:\UKL_INTEGRATION\UPLOADS`. `mode=manifest&days=35` returns branch-scoped populated delivery dates plus replacement revisions, allowing that third machine to repair the oldest missing/changed CSV (including local hash drift) before advancing to the latest non-future transaction date. Requires a numeric `distributor` plus `x-ukl-export-key`/`UKL_SALES_EXPORT_KEY`; branch filtering prevents Nairobi/Nyeri files from containing combined data.
