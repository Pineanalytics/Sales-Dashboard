...
- `POST /api/integrations/alerts/notify` — failure-alert sink for the Windows-scheduled sync scripts that have no SMTP credentials of their own (`scripts/sales-returns-sync.ps1`, `scripts/ukl-sales-export-pull.ps1`), which POST here on failure reusing `UPLOAD_API_KEY`. Forwards to `lib/email.ts`'s `sendOpsAlertEmail`, sent to `OPS_ALERT_EMAIL_TO` via the same SMTP config as the approval-email feature — a no-op (never throws) if either isn't configured, so a missing mail setup never masks the calling script's own error.
