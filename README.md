...
- `GET /api/integrations/ukl/sales-export` — headerless CSV export of one day's, one distributor's `SalesReturnLine` rows (25 columns, matching the original hand-run report's exact order/format), refreshed every five minutes by `scripts/ukl-sales-export-pull.ps1` on the separate server that hosts `D:\UKL_INTEGRATION\UPLOADS`. Smart mode requests an exact Nairobi-calendar yesterday/today manifest and treats a hash-matching file moved into the configured archive folder as successfully delivered; older dates are exported only by an explicit manual `-Date`. Requires a numeric `distributor` plus `x-ukl-export-key`/`UKL_SALES_EXPORT_KEY`; branch filtering prevents Nairobi/Nyeri files from containing combined data.
