...
- `GET /api/integrations/ukl/sales-export` — headerless CSV export of one day's, one distributor's `SalesReturnLine` rows (25 columns, matching the original hand-run report's exact order/format), delivered by `scripts/ukl-sales-export-pull.ps1` on the separate server that hosts `D:\UKL_INTEGRATION\UPLOADS`. Every scheduled or manual write has a separate UTC-date segment followed by a UTC-time-and-GUID run code, so a name can never repeat even if the downstream receiver deletes every prior file. Manual Smart mode checks an exact Nairobi-calendar yesterday/today manifest and only writes changed days. Requires a numeric `distributor` plus `x-ukl-export-key`/`UKL_SALES_EXPORT_KEY`; branch filtering prevents Nairobi/Nyeri files from containing combined data.
